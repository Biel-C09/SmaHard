...
function setup() {
 ...
 shoootFireBallInvisible = createSprite(1060,392,4,5);
 shoootFireBallInvisible.visible = false;
...
function draw() {
  ...
  if(gameState==="depth") {
    ...
    textSize(20);
    fill("black")
    text("aperte espaço para destruir a torre da princesa e a salve, e fuja. ",480,200);
  ...
  if(gameState==="save") {
    ...
    textSize(20);
    fill("black")
    text("fuja com a princesa",500,350);
  ...
  if(jason.isTouching(princessHappy)) {
    princessHappy.y = jason.y+1;
    princessHappy.x = jason.x+55;
    princessHappy.addImage(princessHappImg);
    princessHappy.scale = 0.22;
    princessHappy.depth = jason.depth;
    princessHappy.depth = princessHappy.depth + 10
    cageGround.destroy();
    invisibleEdge1.destroy();
  }
  ...
  if(slime.isTouching(shoootFireBallInvisible)) {
    shoootFireBallL();
  }
  
  if(lifeMonster<=110 && slime.isTouching(portal2)) {
    slime.velocityY = -10;
    slime.velocityX = 0;
  }
